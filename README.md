...
```
😸 专注工作中…              ← 1 个任务在跑（轻眯眼慢浮）
😸 3 个任务并行中…          ← 2~3 个任务（快浮 + 扫视 + 甩汗滴）
😸 5 个任务！忙翻了         ← 4 个以上（急促小抖 + 瞪眼乱瞟 + 双汗滴）
🎉 连击 ×3 完成！！         ← 90 秒内完成 3 个以上（彩带粒子庆祝 8 秒）
🎉 任务完成！               ← 刚完成（5 秒庆祝动画）
😿 任务出错了 ⚠️            ← 有任务失败（颤抖动画）
😺 呼呼大睡 😴 · 5 未读     ← 空闲 ~5 分钟后睡熟：闭眼、呼吸放慢，来任务猛地惊醒
😼 打盹中 zZ · 5 未读       ← 空闲 4 分钟起：眼皮渐垂、呵欠越来越频，困极冒 Zzz
```

菜单栏 `🐾 N 📬M`：N = 执行中任务数，M = 未读完成数。下拉可看清单（**点击直达对应工作区**）、切换 7 款皮肤，**全部矢量动画**、同一套设计语言（渐变身体/表情眼/嘴型/眨眼/呼吸/**眼神跟随**/打呵欠）——球球、饭团（海苔）、菱菱（菱形+腮红）、蛋蛋（呆毛）、咪咪（猫耳+摇尾）、兔兔（长耳）、幽幽（幽灵，波浪裙边+悬浮）；执行中/完成/出错四种状态换情绪体色与表情，执行中再按任务数分三档忙碌动效。形象画法参考 emotion-ball（角色原作者 sam70331），角色设计为原创。

## 功能

| 场景 | 表现 |
|---|---|
| 1 个任务执行中 | 专注动画（轻眯眼慢浮）+ 菜单栏计数 |
| 2~3 个任务并行 | 快浮 + 视线扫两块屏幕 + 太阳穴甩汗滴 |
| 4 个以上任务 | 忙翻：急促小抖 + 瞪眼乱瞟 + 双汗滴 + 波浪嘴，体色同族加深 |
| 单个任务跑太久（10 分钟+） | 不耐烦：绕接地点左右摇摆（跺脚），文案「任务跑了好久了…」 |
| 空闲太久没活干 | 困意渐浓：眼皮下垂、呵欠 22~34s → 8~13s 一次，4 分钟打盹冒 Zzz，~5 分钟后睡熟（闭眼、呼吸放慢）；来任务或鼠标靠近即清醒，熟睡中被叫醒会惊醒弹起 |
| 任务完成 | 🎉 宠物庆祝动画 5 秒 + 菜单栏 `📬M` 未读角标；90 秒内完成 ≥3 个触发**连击彩带**庆祝 8 秒 |
| 任务出错 | 😿 颤抖动画 + `📬M` 未读角标；结束后还会委屈撇嘴一小会儿 |
| 点击宠物 | 弹出任务清单（执行中/完成未读，**点击直达对应工作区**）；悬停/清单弹出期间宠物会安静下来方便点击；**按住可拖动**（拎起会瞪眼 O 嘴，落地松口气），位置自动记忆 |
| 鼠标停在宠物身上 ~1 秒 | 摸头：眯眼笑 + 腮红加深 + 飘小心心 |
| 深夜 / 清晨 | 22~6 点困意来得更快（×2），清晨 6~11 点更精神；开工瞬间小跳 + 金星闪一下 |
...
随便在 ZCode 里发一条消息：宠物应立即变 😸 显示"专注工作中…"；回合结束后宠物跳一段庆祝动画、菜单栏出现 `📬1`。
...
- **球球一族**（`animated` 皮肤）：纯矢量 CoreGraphics 绘制（`PetArt.drawVector`），不走资产；
  由 12fps tick 逐帧重绘实现眨眼（6~14s 随机、过冲回弹）、呼吸（±1%）、嘴型（微笑/抿嘴/大笑/撇嘴/波浪嘴）
  与**眼神跟随**（读全局鼠标，视向平滑逼近；贴近 ~90pt 内瞳孔微放大，>380pt 转为慢速漫游；
  忙碌 2 档以上视线被"工作"接管——在屏幕间扫视/急促乱瞟）。
  idle 呵欠间隔随**困意**缩水（空闲且鼠标不在旁时 4 分钟攒满：22~34s → 8~13s 一次，张嘴挤眼 2.6s，
  困极打盹冒 Zzz，~5 分钟转入深睡闭眼放慢呼吸，来任务或被注视即清醒、熟睡被叫醒会惊醒弹起）。
  执行中按任务数分三档忙碌动效（专注/并行甩汗滴/忙翻），单任务 10 分钟起不耐烦摇摆；
  另有摸头心心、拖拽拎起、连击彩带、开工金星、出错委屈余韵、深夜犯困加倍等小情绪。
  浮动/呼吸用相位积分，深睡降频不跳相位。
  形状家族沿其 blob/wedge/gem 三体型思路：球球（圆）、
  ...
  矢量皮肤可 `--render-art /tmp/pet-art` 导出 PNG 自检（含忙碌 2/3 档与打盹帧）
